...
const JobManagerScreenApp = (props) => {
  ...
  useEffect(() => {
    props.dispatch(action('jobManagerScreenAppModal/init'))

    return () => {
      props.dispatch(action('jobManagerScreenAppModal/clear'))
    }

  }, [])

  useEffect(() => {
    props.dispatch(action('jobManagerScreenAppModal/getJobDetail'))
  }, [refreshCount])
  ...
  function postJobLostFetch () {
    props.dispatch(action('jobManagerScreenAppModal/postJobList', {
      jobNo,
      workNo,
      goods,
      startPointId,
  ...
  function onPause (itemData) {
    props.dispatch(action('jobManagerScreenAppModal/postJobPause', { id: itemData.id }))
  }

  function onContinue (itemData) {
    props.dispatch(action('jobManagerScreenAppModal/postJobContinue', { id: itemData.id }))
  }

  function onCancel (itemData) {
    props.dispatch(action('jobManagerScreenAppModal/postJobCancel', { id: itemData.id }))
  }

  function onItemPress (itemData) {
    props.dispatch(action('jobManagerScreenAppModal/onSelectJob', { selectedJob: itemData }))

  }

  function onSelect (item, type) {
    props.dispatch(action('jobManagerScreenAppModal/save', { [type]: item.id }))
    props.dispatch(action('jobManagerScreenAppModal/postJobList', {
      jobNo,
      workNo,
      goods,
      startPointId,
      endPointId,
      vin: (typeof currentVin !== "undefined" ? currentVin : ""),
      [type]: item.id,
    }));
  }

  function onStartDelivery () {
    props.dispatch(action('jobManagerScreenAppModal/postJobStartDelivery', { id: selectedJobDetail.id }))
  }

  function onUnloaded () {
    props.dispatch(action('jobManagerScreenAppModal/postJobUnload', { id: selectedJobDetail.id }))
  ...
  function reset () {
    ...
    props.dispatch(action('jobManagerScreenAppModal/reset'));
  }
  ...
  return <View style={styles.container}>
    ...
    <View style={styles.content}>
      ...
      <View style={styles.detail}>
          ...
          <ScrollView style={styles.scrollview}>
            {
              jobList?.records?.map((item, i) => {
                ...
                return <JobCard data={item} cardType='app' key={item.id + item.operations?.join('')}
                  selected={selected}
        ...
        {selectedJobDetail && <JobManagerRight
          data={selectedJobDetail} cardType='app'
...
}, configModal: { refreshCount }, userModal: { goodsLists }
}) =>
({
  jobList,
  jobNo,
