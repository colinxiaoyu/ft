...
export const debugHttpServer = 'http://192.168.1.107:8080';
...
const get = async (url, params = {}, headers = {}, baseUrl = getConfig().BASE_URL) => {
  ...
  try {
    const response = await fetchWithTimeout(requestUrl, config);
...
const put = async (url, body = {}, headers = {}, baseUrl = getConfig().BASE_URL) => {
  ...
  const config = {
    ...
    headers: header,
    body: JSON.stringify(body), // 请求体传递数据
  };
...
const handleResponse = async response => {
  ...
  console.log('response url:', response.url, data);
  ...
  return fetch(url, {...config, signal})
    .then(response => {
      ...
      if (config.method === 'GET') {
        console.log('url:', url, 'method:', config.method, 'X-Token:', config.headers['X-Token']);
      } else {
        console.log('url:', url, 'method:', config.method, 'body:', config.body, 'X-Token:', config.headers['X-Token']);
      }
    ...
    .catch(error => {
      ...
      if (error.name === 'AbortError') {
        pubSub.publish('showAlert', '请求超时');
        console.log('url请求超时', url);
